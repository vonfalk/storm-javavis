...
#define COPY(tName, object)						\
	...
	public:
		...
		// Special case allocations, currently only used by the compiler.
		struct unsafe {
			void *mem;
			explicit inline unsafe(void *mem) : mem(mem) {}
		};

		static void *operator new(size_t size, unsafe mem);
		static void operator delete(void *ptr, unsafe mem);
