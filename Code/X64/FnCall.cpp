...
namespace code {
	namespace x64 {
		...
		static Nat pushValue(Listing *dest, const ParamInfo &p) {
			...
			if (p.src.type() != opVariable || p.src.type() != opVariableRef)
		...
		static void setRegisterVal(RegEnv &env, Reg target, Param param, const Operand &src) {
			...
			} else if (src.type() == opVariableRef) {
				Size s(param.size());
				*env.dest << mov(asSize(target, s), xRel(s, src.var(), src.ref(), Offset(param.offset())));
			} else {
