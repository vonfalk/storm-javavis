...
#define TRANSFORM(x) { op::x, &machineX86::x ## Tfm }
		...
		typedef void (*TransformFn)(Listing &, machineX86::TfmParams &, const Instruction &);

		static OpEntry<TransformFn> transformMap[] = {
			TRANSFORM(fnParam),
			TRANSFORM(fnCall),
			TRANSFORM(prolog),
			TRANSFORM(epilog),
		...
		void TfmParams::transform(Listing &to, nat line) {
			static OpTable<TransformFn> transforms(transformMap, ARRAY_SIZE(transformMap));
			...
			TransformFn tfm = transforms[i.op()];
