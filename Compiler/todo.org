...
* Loose ends so far [27/55]
  - [ ] When a .bnf-file includes another package, productions from there are not automatically visible,
    ...
    mechanism.
  - [ ] Allow creating variables from the REPL. We could use global variables inside the ReplSandbox object for this.
  - [ ] Support documentation of templates from C++.
  - [ ] Lambda functions can not infer their proper type when being assigned to Maybe<T>.
  - [ ] Function pointers should perhaps be able to infer the parameter types from context if possible, just like lambdas.
  ...
  - [ ] Global variables should perhaps be initialized lazily. Otherwise, initialization expressions may fail
