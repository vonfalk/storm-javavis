* SMM bugs/improvements [5/11]
  - [ ] SMM crashes fairly consistently when running the test suite with an additional generation of 1 MB.
  ...
  - [ ] Currently, we treat the header pointer of weak objects as a weak pointer. This is not correct,
