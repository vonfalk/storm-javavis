...
* Loose ends so far [37/66]
  - [ ] Asking for a random access stream from an IStream should produce a LazyIStream if nothing better
  ...
  - [ ] The core does not check the return type of overloaded functions, which leads to strange behaviours from time to time...
  - [ ] The Ui library crashes if one uses "kill process" on windows under heavy rendering load.
  - [ ] In Basic Storm, writing 1.2 currently produces a float by default. Is this desirable?
  - [ ] The expression -1 can not be automatically casted to a float since the - operator is used rather than
