...
* Loose ends so far [58/101]
  ...
  - [ ] Interestingly: "strBuf << maybe<Str>" calls via Variant. This might be because the toS()
    template is not instantiated for Str? at that time, and is thus not explored. The same happens
    for this sequence: 10.nat.toS; 10.word.toS; 10.byte.toS; If badness != 0, we should probably try
    to instantiate a template if able to.
