* SMM bugs/improvements [1/9]
  - [ ] GcWatch objects seem to fail to notify their owner of moved objects in rare circumstances. Investigate
  ...
  - [ ] Investigate how often the history should be advanced. It seems to be wasteful to always increase it
    ...
    the nursery a certain number of time, or moved a certain amount of objects.
  ...
  - [ ] Tweak collection frequency. Currently, it seems gen 1 collections are always triggered because
  ...
  - [ ] Utilize memory protection (write barriers) to improve performance.
  - [ ] We probably want a fenwich tree to provide quick range queries regarding write barriers (perhaps
    other operations as well) in the VMAlloc system.
  ...
  - [ ] Investigate where we should put 'suggestCollection' in 'allocBlock'. Currently, we're a bit aggressive.
