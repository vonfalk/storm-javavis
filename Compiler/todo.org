...
* Loose ends so far [57/101]
  ...
  - [ ] When exporting something in the current package, do we want to make sure that those are
    visible in the same package as well? Currently both BS and BNF ignores them (which is
    sensible). Having this feature introduces many interesting circular dependencies - parsing a
    file to find exports likely needs to know the exports of other files in the same package.
  - [ ] Add some helper "rules" to the recursive descent parser. For example: Capture(Nat n) that
