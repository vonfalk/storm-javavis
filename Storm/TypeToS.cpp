...
namespace storm {
	...
	CodeGen *TypeToS::generateCode() {
		...
		Function *addChar = as<Function>(strBufType.type->findCpp(L"addChar", valList(2, strBufType, natType)));
		Function *addStr = as<Function>(strBufType.type->findCpp(L"add", valList(2, strBufType, strType)));
		...
		for (nat i = 0; i < vars.size(); i++) {
			...
			Function *f = as<Function>(storm::find(type.type, name));
			if (!f)
				f = as<Function>(storm::find(engine().package(L"core"), name));
		...
		Function *toS = as<Function>(strBufType.type->findCpp(L"toS", valList(1, strBufType)));
