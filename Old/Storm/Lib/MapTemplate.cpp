...
namespace storm {
	...
	bool MapIterType::loadAll() {
		...
		add(steal(nativeFunction(e, ref, L"++*", valList(1, ref), address(&MapBase::Iter::preIncRaw))));
		add(steal(nativeFunction(e, val, L"*++", valList(1, ref), address(&MapBase::Iter::postIncRaw))));
