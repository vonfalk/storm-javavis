...
public:
	...
	Value(const Fn<T> &getter, const Fn<void, T> &setter);
	Value(const Fn<const T &> &getter, const Fn<void, const T &> &setter);
...
private:
	...
	T *data;
	Fn<T> *plainGetter;
	Fn<void, T> *plainSetter;
	Fn<const T &> *refGetter;
	Fn<void, const T &> *refSetter;
...
template <class T>
Value<T> *value(const Fn<T> &getter, const Fn<void, T> &setter) {
	return new Value<T>(getter, setter);
}

template <class T>
Value<T> *value(const Fn<const T &> &getter, const Fn<void, const T &> &setter) {
	return new Value<T>(getter, setter);
}
...
Value<T>::Value(const Fn<T> &getter, const Fn<void, T> &setter) {
	zeroMembers();
	this->plainGetter = new Fn<T>(getter);
	this->plainSetter = new Fn<void, T>(setter);
}

template <class T>
Value<T>::Value(const Fn<const T &> &getter, const Fn<void, const T &> &setter) {
	zeroMembers();
	this->refGetter = new Fn<const T&>(getter);
	this->refSetter = new Fn<void, const T&>(setter);
}
